    ...
    : juce::AudioProcessorEditor (&p), audioProcessor (p), pipeEditor (p)
{
    ...
    pingPongAttachment = std::make_unique<ButtonAttachment> (apvts, ParamIDs::pingPong, pingPongButton);
...
void HadenianMonoDelayAudioProcessorEditor::paint (juce::Graphics& g)
{
    ...
    g.setFont (juce::Font (titleBar.getHeight() * 0.42f, juce::Font::bold));
    g.drawText ("Hadenian", titleBar.withTrimmedLeft (8), juce::Justification::centredLeft);

    auto wordmarkArea = titleBar.removeFromRight ((int) (getWidth() * 0.32f));
    juce::Font monoFont (titleBar.getHeight() * 0.42f, juce::Font::bold);
    juce::Font delayFont (titleBar.getHeight() * 0.42f, juce::Font::plain);
    g.setFont (monoFont);
    int monoWidth = monoFont.getStringWidth ("mono") + 4;
    g.setColour (HadenianColours::textDark);
    g.drawText ("mono", wordmarkArea.removeFromLeft (monoWidth), juce::Justification::centredLeft);
    g.setFont (delayFont);
    g.setColour (HadenianColours::textMuted);
    g.drawText ("delay", wordmarkArea, juce::Justification::centredLeft);
}
...
void HadenianMonoDelayAudioProcessorEditor::resized()
{
    ...
    auto topRow = bounds.removeFromTop ((int) (getHeight() * 0.205f));
    bounds.removeFromTop ((int) (getHeight() * 0.012f));
    ...
    // --- Top row: DELAY (+SYNC), FEEDBACK, MIX ---
    ...
    // --- Bottom row: LO FILTER, HI FILTER, PING PONG, WIDTH ---
    int knobSize2 = (int) (getWidth() * 0.115f);
    int colW2 = bottomRow.getWidth() / 4;

    auto loCol    = bottomRow.removeFromLeft (colW2);
    auto hiCol    = bottomRow.removeFromLeft (colW2);
    auto ppCol    = bottomRow.removeFromLeft (colW2);
    ...
    auto ppArea = ppCol.withSizeKeepingCentre (juce::jmin (ppCol.getWidth() - 10, 120), 26);
    pingPongButton.setBounds (ppArea);
