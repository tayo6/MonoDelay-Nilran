...
void MultiTapDelay::setPingPong (bool shouldPingPong) { pingPongOn.store (shouldPingPong); }
...
void MultiTapDelay::updateFilterCoefficients()
{
    ...
    loFilterProcL.coefficients = lo;
    loFilterProcR.coefficients = lo;
    hiFilterProcL.coefficients = hi;
    hiFilterProcR.coefficients = hi;
}
...
void MultiTapDelay::process (juce::AudioBuffer<float>& buffer)
{
    ...
    for (int n = 0; n < numSamples; ++n)
    {
        ...
        float pathA = 0.0f; // even-indexed taps (or all taps, if ping-pong is off)
        float pathB = 0.0f; // odd-indexed taps (only used when ping-pong is on)
        ...
        for (int i = 0; i < N; ++i)
        {
            ...
            float contribution = sample * gain;
        ...
        float filteredA = hiFilterProcL.processSample (loFilterProcL.processSample (pathA));
        float filteredB = pp ? hiFilterProcR.processSample (loFilterProcR.processSample (pathB)) : filteredA;

        float feedbackSignal = filteredA + (pp ? filteredB : 0.0f);
        delayData[writePos] = monoIn + feedbackSignal;
        writePos = (writePos + 1) % bufferLength;

        float mid  = 0.5f * (filteredA + filteredB);
        float side = 0.5f * (filteredA - filteredB) * curWidth;
        float wetL = mid + side;
        float wetR = mid - side;
